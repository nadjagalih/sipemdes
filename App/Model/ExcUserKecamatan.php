<?php
session_start();
error_reporting(E_ALL ^ E_NOTICE);

include "../../Module/Config/Env.php";

if (empty($_SESSION['NameUser']) && empty($_SESSION['PassUser'])) {
    $logout_redirect_url = "../../Auth/SignIn?alert=SignOutTime";
    header("location: $logout_redirect_url");
} else {
    if ($_GET['Act'] == 'Save') {
        if (isset($_POST['Save'])) {
            $ViewTanggal   = date('YmdHis');
            $UserNama = sql_injeksi($_POST['UserNama']);
            $Pass  = sql_injeksi(password_hash($_POST['Pass'], PASSWORD_DEFAULT));
            $Akses = sql_injeksi($_POST['Akses']);
            // $NIK = sql_injeksi($_POST['NIK']);
            $Nama = sql_injeksi($_POST['Nama']);
            $Kecamatan = sql_injeksi($_POST['IdKecamatan']);
            $StatusLogin = sql_injeksi($_POST['Status']);
            $Setting = 1;

            $CekUser = mysqli_query($db, "SELECT NameAkses FROM main_user_kecamatan WHERE NameAkses='$UserNama' ");
            $Row = mysqli_num_rows($CekUser);
            if ($Row <> 0) {
                header("location:../../View/v?pg=UserViewKecamatan&alert=CekUser");
            } else {
                $CekPassword = sql_injeksi($_POST['Pass']);
                if (strlen($CekPassword) >= 5) {

                    // $QUser = mysqli_query($db, "SELECT * FROM main_user");
                    // $Count = mysqli_num_rows($QUser);
                    // if ($Count <> 0) {
                    //     $TempId = $Count + 1;
                    //     $IdUser = $ViewTanggal . "" . $TempId;
                    // } else {
                    //     $TempId = 1;
                    //     $IdUser = $ViewTanggal . "" . $TempId;
                    // }

                    $tanggal        = date('Ymd');
                    $waktuid        = date('His');
                    $IdUser = $tanggal . "" . $waktuid . "" . substr($UserNama, -3);

                    $Save = mysqli_query($db, "INSERT INTO main_user_kecamatan(IdUser, NameAkses, NamePassword, IdLevelUserFK, Status, IdKecamatanFK, StatusLogin, Nama)
                    VALUE('$IdUser','$UserNama','$Pass','$Akses','$Setting','$Kecamatan','$StatusLogin','$Nama')");

                    if ($Save) {
                        header("location:../../View/v?pg=UserViewKecamatan&alert=Save");
                    }
                } elseif (strlen($CekPassword) < 5) {
                    header("location:../../View/v?pg=UserViewKecamatan&alert=Karakter");
                }
            }
        }
    } elseif ($_GET['Act'] == 'Edit') {
        if (isset($_POST['Edit'])) {
            $IdUser = sql_injeksi($_POST['IdUser']);
            $UserNama = sql_injeksi($_POST['UserNama']);
            $Pass = sql_injeksi($_POST['Pass']);
            $Akses = sql_injeksi($_POST['Akses']);
            // $NIK = sql_injeksi($_POST['NIK']);
            $Nama = sql_injeksi($_POST['Nama']);
            $Kecamatan = sql_injeksi($_POST['IdKecamatan']);
            $Status = sql_injeksi($_POST['Status']);

            $Edit = mysqli_query($db, "UPDATE main_user_kecamatan SET NameAkses = '$UserNama',
            IdLevelUserFK = '$Akses',
            Nama = '$Nama',
            IdKecamatanFK = '$Kecamatan',
            StatusLogin = '$Status'
            WHERE IdUser = '$IdUser' ");

            // if ($Status == 0) {
            //     $EditStatusPegawai = mysqli_query($db, "UPDATE master_pegawai SET Setting = '$Status'
            // WHERE IdPegawaiFK = '$IdPegawaiFK' ");
            // } elseif ($Status == 1) {
            //     $EditStatusPegawai = mysqli_query($db, "UPDATE master_pegawai SET Setting = '$Status'
            // WHERE IdPegawaiFK = '$IdPegawaiFK' ");
            // }

            if ($Edit) {
                header("location:../../View/v?pg=UserViewKecamatan&alert=Edit");
            }
        }
    } elseif ($_GET['Act'] == 'Delete') {
        if (isset($_GET['Kode'])) {
            $IdUser = sql_injeksi(($_GET['Kode']));
            $Delete = mysqli_query($db, "DELETE FROM main_user_kecamatan WHERE IdUser = '$IdUser' ");

            if ($Delete) {
                header("location:../../View/v?pg=UserViewKecamatan&alert=Delete");
            }
        }
    } elseif ($_GET['Act'] == 'Reset') {
        if (isset($_POST['Reset'])) {

            $IdUser = sql_injeksi($_POST['IdUser']);
            $Pass = sql_injeksi(password_hash($_POST['Pass'], PASSWORD_DEFAULT));
            $EditReset = mysqli_query($db, "UPDATE main_user_kecamatan SET NamePassword = '$Pass'
            WHERE IdUser = '$IdUser' ");

            if ($EditReset) {
                header("location:../../View/v?pg=UserViewKecamatan&alert=Reset");
            }
        }
    }
}
