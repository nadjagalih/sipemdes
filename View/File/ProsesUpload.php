...
if (!in_array(strtolower($ext), $allowed)) {
    header("Location: ../v?pg=FileViewAdmin&alert=FileExt");
}
...
if ($iddesa) {
    ...
    $sql = "INSERT INTO file (Nama, Ekstensi, FileBlob, IdFileKategoriFK, IdLevelFileFK)
            VALUES ('$nama', '$ext', '$binaryData', '$kategori', 1)";
}

$maxFileSize = 5 * 1024 * 1024; // 5MB in bytes
if ($file['size'] <= $maxFileSize) {
    if (mysqli_query($db, $sql)) {
        header("Location: ../v?pg=FileViewAdmin&alert=UploadSukses");
        exit();
    } else {
        echo "Database error: " . mysqli_error($db);
    }
} else {
    header("Location: ../v?pg=FileViewAdmin&alert=FileMax");
}
?>