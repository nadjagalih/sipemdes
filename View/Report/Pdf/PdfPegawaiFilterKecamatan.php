...
if (isset($_GET['Proses']) && isset($_GET['Kecamatan']) && !empty($_GET['Kecamatan'])) {
    ...
    $content =
        '<html>
            <body>
            <h4>Data Kepala Desa & Perangkat Desa Kecamatan' . " " . $NamaKecamatan . '<br>Pertanggal ' . $Tanggal_Cetak . ' Pukul ' . $Waktu_Cetak . ' WIB</h4>
    ...
    while ($DataPegawai = mysqli_fetch_assoc($QueryPegawai)) {
        ...
        $content .=
            '<td width="100">' . $Kecamatan . '<br><strong>' . $NamaDesa . '</strong><br>' . $KodeDesa . '</td>';
        if (empty($Foto)) {
            $content .=
        ...
        $content .=
            '<td width="80">' . $Pendidikan . '</td>
                ...
                <td width="70" align="right">' . $Siltap . '</td>
                <td width="90" align="left">' . $Telp . '</td>
            </tr>';
        $Nomor++;
    }
    $content .= '</tbody>
