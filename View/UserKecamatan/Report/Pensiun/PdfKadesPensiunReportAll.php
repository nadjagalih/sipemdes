...
if (!empty($filterDesa)) {
    ...
    $title = 'Data Kepala Desa Mendekati Masa Pensiun ' . $filterDesa . ' Kecamatan ' . $NamaKecamatan;
    $filename = 'Data Kades Pensiun Desa ' . $filterDesa . ' Kecamatan ' . $NamaKecamatan . '_' . $DateCetak;
} else {
    $title = 'Data Kepala Desa Mendekati Masa Pensiun Kecamatan ' . $NamaKecamatan;
    $filename = 'Data Kades Pensiun Kecamatan ' . $NamaKecamatan . '_' . $DateCetak;
}
            ...
            <table border="0.3" cellpadding="2" cellspacing="0" width="100%">
                <thead>
                    <tr align="center">
                        ...
                        <th><strong>Tgl Mutasi</strong></th>
                        <th><strong>Tgl Pensiun<br>Sisa Pensiun</strong></th>
...
while ($DataPegawai = mysqli_fetch_assoc($QueryPegawai)) {
    ...
    // Calculate pension date (6 years from mutation date for Kades)
    ...
    if ($TglSekarang1 >= $TanggalPensiun && $Setting == 1) {
        if (!is_null($IdFilePengajuanPensiunFK) && $StatusPensiunDesa === '1') {
            $keterangan = 'File Pengajuan Ada';
        } else if (!is_null($IdFilePengajuanPensiunFK) && $StatusPensiunDesa === null) {
            $keterangan = 'Menunggu Persetujuan Desa';
        } else if ($StatusPensiunDesa === '0') {
            $keterangan = 'Pengajuan Ditolak Desa';
        } else {
            $keterangan = 'Pensiun Belum Mengajukan';
        }

        if ($StatusPensiunKecamatan === '1') {
            $keterangan .= ' - Disetujui Kecamatan';
        } elseif ($StatusPensiunKecamatan === '0') {
            $keterangan .= ' - Ditolak Kecamatan';
        }
    } else {
        $keterangan = 'Belum Pensiun';
    }

    // Add background color for urgent cases
    $rowStyle = '';
    if ($TahunInt == 0 && $BulanInt < 3) {
        $rowStyle = ' style="background-color: #fecaca;"';
    }
    ...
    $content .= '<br>' . $JenisKelamin . '</td>
            <td width="80">' . $ViewTglMutasi . '</td>
            <td width="160">' . $ViewTglPensiun . '<br>' . $HasilTahun . " " . $HasilBulan . " " . $HasilHari . '</td>
